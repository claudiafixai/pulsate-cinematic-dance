...
const SectionHeading = ({ tag, title, description, center = false }: SectionHeadingProps) => {
  return (
    <div className={`mb-12 ${center ? "text-center" : ""}`}>
      ...
      {description && (
        <p className={`mt-4 text-muted-foreground max-w-2xl leading-relaxed text-base sm:text-lg ${center ? "mx-auto" : ""}`}>
          {description}
        </p>
      )}
      <div className={`divider-gold mt-6 max-w-[80px] ${center ? "mx-auto" : ""}`} />
    </div>
  );
};
